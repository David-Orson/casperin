...
func ProbeWDL(pos *backend.Position, depth int) int {
	return int(C.tb_probe_wdl(
		C.ulong(pos.Colours[backend.White]),
		C.ulong(pos.Colours[backend.Black]),
		C.ulong(pos.Pieces[backend.King]),
		C.ulong(pos.Pieces[backend.Queen]),
		C.ulong(pos.Pieces[backend.Rook]),
		C.ulong(pos.Pieces[backend.Bishop]),
		C.ulong(pos.Pieces[backend.Knight]),
		C.ulong(pos.Pieces[backend.Pawn]),
		C.uint(0),
		C.uint(0),
		C.uint(0),
