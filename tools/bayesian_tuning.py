...
from os import path, chdir, system, pipe, read, remove
...
def build_engine(exe_name):
    chdir('..')
    system('go build -o {} casperin.go'.format(exe_name))
    chdir('./tools')
    

def optimize_parameter(parameters: List[SearchConstant]):
    build_engine('casperin')
